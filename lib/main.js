...
function changePassword(siteURL, callbackMap, newPassword, username) {
    ...
    // Add common behaviors to failure callback
    // (Will do the same to success callback later on.)
    ...
    if(siteURL in AUTOMATION_CONFIG) { // Set up and launch automation
        ...
        (function() { // Only include username in the search if we were given it.
            ...
            onComplete: function(credentials) {
                ...
                // Add common behaviors to success callback
                var originalSuccess = callbackMap.success || function(){};
                callbackMap.success = function() {
                    reEnableInPasswordManager(siteURL);

                    // Update password manager with new password
                    var newCredential = credentials[0];
                    newCredential.password = newPassword;
                    passwords.store(newCredential);
                    // FIXME: this produces an exception
                    // [Exception... "'This login already exists.' when calling method: [nsILoginManager::addLogin]
                    // Need to use nsILoginManager.modifyLogin instead
